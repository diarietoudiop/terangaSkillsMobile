...
class AdministrativeRequestRepository {
  ...
  Future<AdministrativeRequestModel> createRequest({
    ...
    final response = await _apiClient.createAdminRequest(
      formData,
  ...
  Future<List<AdministrativeRequestModel>> getAllRequests() async {
    ...
    final List list = (data is Map)
        ? (data['data'] ??
              data['requests'] ??
              data['administrativeRequests'] ??
              [])
        : (data as List? ?? []);
    return list
        .map(
          (e) => AdministrativeRequestModel.fromJson(e as Map<String, dynamic>),
        )
        .toList();
  }
  ...
  Future<void> deleteRequest(String id) => _apiClient.deleteAdminRequest(id);

  Future<void> generateDocument(String id, String name, String content) async {
    await _apiClient.generateDocument(id, name, content);
  }
}
